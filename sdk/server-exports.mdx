...
description: "Upload, list, delete and purge FiveMesh CDN objects from FiveM server scripts."
icon: "server"
---
...
if not result.success then
    print(("FiveMesh bulk upload failed: %s"):format(result.error.message))
    return
end
...
if not nextPage.success then
    print(("FiveMesh list failed: %s"):format(nextPage.error.message))
    return
end

print(nextPage.continuationToken)
```
