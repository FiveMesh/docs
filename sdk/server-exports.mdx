...
The service key is read from `FIVEMESH_API_KEY`, so these exports should be called from server-side Lua or JavaScript.
...
| `bulkDelete(paths)` | Deletes multiple object paths and returns per-path results. |
